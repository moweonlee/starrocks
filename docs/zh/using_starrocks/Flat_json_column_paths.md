...
강제 경로가 사용자 지정 순서대로 좌→우로 budget 을 먼저 소비하고, 남은 slot 에
자동 감지된 컬럼이 채워집니다. 강제 경로 수가 budget 을 초과하면 사용자 지정
순서상 앞쪽 경로가 유지되고 나머지는 잔여(remainder) JSON blob 으로 강등됩니다.

## 중첩·비정형 JSON 에서의 동작

강제 경로는 JSON 문서 내부를 가리키는 점(`.`)으로 구분된 경로이므로 `$.address.city`
같은 중첩 리프도 지원합니다. 엔진은 경로를 실제 문서 구조와 대조하며, 데이터가
경로와 맞지 않아도 안전하게 폴백합니다:

| 상황                                                                 | 동작                                                                                 |
|----------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| 강제 경로가 중첩 리프(`$.a.b.c`)를 가리킴                            | 해당 리프가 서브컬럼으로 materialize 됨.                                             |
| 강제 경로가 리프가 아닌 객체(`$.a` = `{...}`)를 가리킴               | 그 객체의 자식 리프들이 materialize 됨.                                              |
| 강제 리프의 상위 노드가 일부 행은 스칼라, 일부 행은 객체            | 충돌 노드를 하나의 JSON 서브컬럼으로 materialize; 쿼리 결과는 여전히 정확.           |
| 강제 경로가 데이터보다 깊거나, 키가 전혀 등장하지 않음              | 서브컬럼을 만들지 않음 (빈 컬럼 없음).                                               |
| 어떤 행에서 경로가 없음 / `null` / `{}` / JSON 값 전체가 `NULL`      | 안전하게 처리되어 `NULL` 로 조회됨.                                                  |

키는 **대소문자를 구분**하여 정확히 매칭하므로 `$.Browser` 와 `$.browser` 는 서로 다른
경로입니다. `column_paths` 를 변경해도 기존 데이터를 다시 쓰거나 쿼리 결과를 바꾸지
않으며, 이후에 쓰이는 세그먼트(및 컴팩션된 기존 데이터)의 저장 레이아웃만 바뀝니다.
서로 다른 `column_paths` 설정으로 쓰인 세그먼트들이 공존해도 쿼리는 모두 동일한
결과를 반환합니다.

## 성능 영향

hot 하지만 희소한 경로를 타입이 있는 서브컬럼으로 강제하면 읽기 시점의 행별 JSON
파싱을 피하고, 플래너가 술어(predicate)를 스토리지로 push down 할 수 있습니다. 이는
적재/컴팩션 시점의 약간의 추가 작업을 대가로, 희소성 휴리스틱이라면 잔여 JSON blob 에
남겨둘 경로에 대해 큰 읽기 성능 이득을 얻는 trade-off 입니다.

단일 tablet 예시 측정(30만 행, 강제 경로가 25% 행에만 존재 — 희소성 임계치 미만이라
`column_paths` 없이는 잔여에 남음)에서, 해당 경로를 필터·추출하는 쿼리가 수 배 빨라졌고
tablet 크기도 눈에 띄게 작아졌습니다. 실제 수치는 데이터 형태·값 폭·쿼리 패턴에 따라
달라집니다.

이 이득은 tablet 당 행 수에 비례하므로, 과도한 파티셔닝이나 버킷 과다 설정은 tablet 당
행 수를 줄여 이득을 희석시킵니다. 실제로 hot 한 경로만 강제하십시오. 강제 서브컬럼은
각각 `flat_json.column.max` budget 을 소비합니다.

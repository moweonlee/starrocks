...
void Tablet::update_flat_json_config(const FlatJsonConfig& new_config) {
    // Version-based gating mirrors update_binlog_config: ignore stale or duplicate pushes so
    // a slow tablet report retry cannot regress a newer applied config. _tablet_meta does the
    // actual copy into a fresh shared_ptr (see TabletMeta::set_flat_json_config).
